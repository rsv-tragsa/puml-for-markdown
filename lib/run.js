...
const run = async (options = {}) => {
  const configuredRootDirectory = path.resolve(options.rootDirectory ?? DEFAULT_CONFIG.rootDirectory)
  if (!fs.existsSync(configuredRootDirectory)) throw new Error(`rootDirectory does not exist: ${configuredRootDirectory}`)
  const rootDirectory = fs.realpathSync(configuredRootDirectory)
  const relativeToCanonicalRoot = (value) => {
    if (typeof value !== 'string') return value
    const absolute = path.resolve(configuredRootDirectory, value)
    return isPathInside(configuredRootDirectory, absolute)
      ? path.relative(configuredRootDirectory, absolute)
      : value
  }
  const pumlDirectory = resolveConfiguredDirectory(
    rootDirectory,
    relativeToCanonicalRoot(options.pumlDirectory ?? DEFAULT_CONFIG.pumlDirectory),
    'pumlDirectory',
  )
  const markdownDirectory = resolveConfiguredDirectory(
    rootDirectory,
    relativeToCanonicalRoot(options.markdownDirectory ?? DEFAULT_CONFIG.markdownDirectory),
    'markdownDirectory',
  )
  const distDirectory = resolveConfiguredDirectory(
    rootDirectory,
    relativeToCanonicalRoot(options.distDirectory ?? DEFAULT_CONFIG.distDirectory),
    'distDirectory',
  )
  ...
  const gitignorePath = path.resolve(
    rootDirectory,
    relativeToCanonicalRoot(options.gitignorePath ?? DEFAULT_CONFIG.gitignorePath),
  )
  ...
  const requestedPuml = validateFileSelection({
    files: options.pumlFiles?.map(relativeToCanonicalRoot),
    rootDirectory,
  ...
  const requestedMarkdown = validateFileSelection({
    files: options.markdownFiles?.map(relativeToCanonicalRoot),
    rootDirectory,
